...
export async function createUsuarioWithRole(empresaId: any, payload: UsuarioInput) {
  ...
  const userPayload = {
    ...
    // password handling: if provided, hash it and save into password column
    // Note: the DB must have a password column in `usuario` for this to persist
    password: payload.password ? await (await import('./authService')).hashPassword(payload.password) : null,
  };
